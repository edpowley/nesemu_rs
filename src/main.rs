...
fn run_test_rom() {
    ...
    let rom_state = emulator::RomState::load(rom_path);
    let mut emu_state = emulator::EmuState::new(&rom_state);
...
fn run_game() {
    ...
    loop {
        emu_state.run_to_next_nmi();
        println!("{} {}", emu_state.ppu_x, emu_state.ppu_y);
    }
}
